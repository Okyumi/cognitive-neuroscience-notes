...
![Brain diagram showing inferior longitudinal fasciculus (blue), occipital face area (orange), and fusiform face area (yellow)](images/pdf1-287.ppm)
...
![Left: (a) ECoG electrode placement on cortical surface (face-selective patches marked, signal-to-noise ratio shown); Center: (b) fMRI face-selective maps with polar angle; Right: Visual areas map (FFA, EBA, PPA, VWFA, LO) with face/body/place/word stimuli images](images/pdf1-288.ppm)
...
![Row a (top): Many individual face stimuli with spike rate recordings; Row b (top right): histogram of spike counts per picture number showing selective high firing for ~picture 30 (Jennifer Aniston cell)](images/pdf1-289.ppm)

![Row a (bottom): More individual face stimuli with corresponding recordings](images/pdf1-290.ppm)
...
![Three images: grayscale face photo (left), face photo with white dot landmark markers (center), scatter plot of facial landmarks in PC space (right)](images/pdf1-291.ppm)
...
![Left panel: Four columns of object category images (faces, bodies, tools, etc.) in green/pink/blue/yellow frames; Right panel: Brain activation map labeled "Response to Faces" with Even Runs highlighted](images/pdf1-292.ppm)

![Left: Bar graph showing within-category correlations (red) vs. between-category correlations (blue) across All object-selective cortex, Excluding maximally-responsive voxels; Right: images of face/body/tool stimulus categories](images/pdf1-293.ppm)
...
![Left brain image showing VWFA (Visual Word Form Area) and Face area (FFA) locations with "Table" word and face stimuli; Right: diagram of left and right hemisphere connectivity — language/semantics connections, left fusiform/right fusiform areas, peripheral and central processing pathways](images/pdf1-294.ppm)
...
![Step 1: Recording responses to parameterized faces from macaque face patches; Step 2: Single cells tuned to single face axes, blind to changes orthogonal to the axis (3D cube diagram of face axis 1, 2, 3 with firing rate color map); Step 3: Axis model allows precise encoding and decoding of neural responses (encoder/decoder diagram with actual face and predicted face)](images/pdf1-323.ppm)

![Pairs of faces differing along face axes — showing how neurons tuned to axes respond similarly to faces with same axis values](images/pdf1-324.ppm)

![Actual face vs. Predicted face (all) — model reconstruction results](images/pdf1-325.jpg)
